...
public class PresetEmojiLoader {
    ...
    static void init(Context context, EmojiData emojiData) {
        PresetEmojiLoader.context = context;
    ...
    public interface EmojiLoaderListener {
        void onEmojiLoaded(PresetEmoji emoji);
    }
        ...
        if (emojiBmp[page][page2] == null) {
            ...
            globalQueue.postRunnable(() -> {
                ...
                uiThread.post(() -> {
                    ...
                    if (listener != null)
                        listener.onEmojiLoaded(new PresetEmoji(code, SmojiManager.getInstance().getEmojiData()));
                });
            });
        } else {
            if (listener != null)
        ...
        if (loadingListeners != null && loadingListeners.size() > 0) {
            ...
            for (ListenerData data : loadingListeners) {
                if (data.code.equals(code) && data.listener != null && data.listener != listener) {
                    data.listener.onEmojiLoaded(new PresetEmoji(code, SmojiManager.getInstance().getEmojiData()));
                    remove.add(data);
                }
            }
            if (remove.size() > 0) {
    ...
    private static void loadEmojiInternal(final byte page, final short page2) {
        try {
            ...
            emojiBmp[page][page2] = SmojiManager.getInstance().getEmojiData()
    ...
    public static String fixEmoji(String emoji) {
        ...
        for (int a = 0; a < length; a++) {
            ...
            if (ch >= 0xD83C && ch <= 0xD83E) {
                ...
                if (SmojiManager.getInstance().getEmojiData().getEmojiToFE0FMap().containsKey(ch)) {
    ...
    private static DrawableInfo getDrawableInfo(CharSequence code) {
        ...
        if (info == null) {
            CharSequence newCode = SmojiManager.getInstance().getEmojiData().getEmojiAliasMap().get(code);
            if (newCode != null) {
                info = PresetEmojiLoader.rects.get(newCode);
            }
        }
        return info;
    }
    ...
    public static Drawable getEmojiBigDrawable(String code) {
        ...
        if (ed == null) {
            CharSequence newCode = SmojiManager.getInstance().getEmojiData().getEmojiAliasMap().get(code);
            if (newCode != null) {
                ed = PresetEmojiLoader.getEmojiDrawable(newCode);
            }
        }
        if (ed == null) {
            return null;
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (c >= 0xD83C && c <= 0xD83E || (buf != 0 && (buf & 0xFFFFFFFF00000000L) == 0 && (buf & 0xFFFF) == 0xD83C && (c >= 0xDDE6 && c <= 0xDDFF))) {
                    ...
                    if (i > 0) {
                        ...
                        && SmojiManager.getInstance().getEmojiData().getDataCharsMap().containsKey(c)) {
                    if (startIndex == -1) {
                        startIndex = i;
                    }
                    startLength++;
                ...
                if (doneEmoji) {
                    ...
                    drawable = PresetEmojiLoader.getEmojiDrawable(code);
                    if (drawable != null) {
